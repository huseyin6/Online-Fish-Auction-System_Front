...
function FishermanFishesPage() {
  ...
  */
  ...
  const handleGetAllFish = async (id) => {
    setLoading(true);
    try {
      const response = await axios.get(`fish/getFishesByFishermanId/${id}`);
      if (response.status === 201) {
        let temp = response.data;
...
export default FishermanFishesPage;